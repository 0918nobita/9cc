...
let generate_code ast =
  let rec generate_inner_code ast = match ast with
      Add (lhs, rhs) -> multi_line_string [
          generate_inner_code lhs;
          generate_inner_code rhs;
          "  pop rdi";
          "  pop rax";
          "  add rax, rdi";
          "  push rax";
        ]
    | Sub (lhs, rhs) -> multi_line_string [
          generate_inner_code lhs;
          generate_inner_code rhs;
          "  pop rdi";
          "  pop rax";
...
let () =
  match expr Sys.argv.(1) 0 with
      Success (ast_list, _, _) ->
        if List.length ast_list = 0
          then (Printf.fprintf stderr "パースに失敗しました"; exit 1)
          else print_string @@ generate_code @@ List.hd ast_list
    | Failure -> (Printf.fprintf stderr "パースに失敗しました"; exit 1)
